...
describe('localStorageUtils', () => {
  describe('setKey()', () => {
    it(`should set "${girlFriendValue1}" to key "${girlFriendKey}"`, () => {
      ...
      expect(localStorage.__STORE__[girlFriendKey]).toBe(girlFriendValue1.toString());
    });
    ...
    it(`should update "${girlFriendValue2}" to key "${girlFriendKey}"`, () => {
      ...
      expect(Object.keys(localStorage.__STORE__).length).toBe(1);
      expect(localStorage.__STORE__[girlFriendKey]).toBe(girlFriendValue2.toString());
    });
    ...
    it(`should set "${age}" to key "${ageKey}"`, () => {
      ...
      expect(localStorage.__STORE__[ageKey]).toBe(age.toString());
      expect(localStorage.__STORE__[girlFriendKey]).toBe(girlFriendValue2.toString());
    });
  });
  ...
  describe('getKey()', () => {
    it(`should return "${age}" for key "${ageKey}"`, () => {
      expect(localStorageUtils.getKey(ageKey)).toBe(age.toString());
  ...
  describe('removeKey()', () => {
    it(`should remove key "${ageKey}" from local storage`, () => {
      ...
      expect(Object.keys(localStorage.__STORE__).length).toBe(1);
      expect(localStorage.__STORE__[ageKey]).toBe(undefined);
    });
  });
